...
float inchHg = 3386.39;
float pressure;
float barmili;		//To store the barometric pressure (Pa)
float temperature;	//To store the temperature (oC)
int altimeter; 		//To store the altimeter (m) (you can also use it as a float variable)

void setup() {
  bmp.begin();		//Begin the senso
...
void loop() {
  ...
  barmili = (pressure/inchHg);
  temperature = bmp.readTemperature();
  altimeter = bmp.readAltitude (35.23); //Change the "1050.35" to your city current barrometric pressure (https://www.wunderground.com)
  //Print values to serial monitor:
  Serial.print(F("Pressure: "));
  Serial.print(barmili);
